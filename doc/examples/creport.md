...
```
           ensure value_lt(w, I, V) <-> (I ~= i & value_lt(old w, I, V)) | (I = i & v < V);
           ensure value_lte(w, I, V) <-> (I ~= i & value_lte(old w, I, V)) | (I = i & v <= V);
```
        }
...
This is provable, but are you sure you want it as a post-condition? It
could instead be stated as a property, since it is always true.
...
```
Trick: adding an empty proof here causes this property to be used as
an assumption when checking the later properties.
```
...
```
Another dirty trick: a constructor function for record would create a
function cycle. For this reason we instead use a constructor axiom
that we can apply to construct structures from given arguments.
We should have a nice syntax for the following, but for now this is
the only way.
