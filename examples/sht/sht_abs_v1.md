...
# model to a sort of toy TCP. There are a few conjectures
# in the net implementation that have not been checked.
...
```
    ...
    conjecture low.sent(M) & net_msg_mtype(M) = request_t & net_msg_dst(M) = D
               & net_msg_src(M) = S & neti(D).recv_seq(S) <= net_msg_seq_num(M)
               -> net.requested(D,net_msg_req(M))

    conjecture low.sent(M) & net_msg_mtype(M) = delegate_t & net_msg_dst(M) = D
               & net_msg_src(M) = S & neti(D).recv_seq(S) <= net_msg_seq_num(M)
               -> net.delegated(D,net_msg_shard(M))
