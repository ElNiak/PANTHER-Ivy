...
```
#######################################
Each Acquire has a transaction ID (txid). This map tracks the
txid associated with accepted and finishing acquires for a given
...
```
(3) May not use a txid in use by a requested acquire
...
(1) A release ack implies a requested release of the
given word.
...
```
(6) Cannot grant if the txid if already in use for a
