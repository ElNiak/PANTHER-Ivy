  ...
  struct my_manager_port : public manager_port {
    ...
    bool get_release_ready(){
      ...
      bool get_grant(grant &a){
        if (dut.L2Unit__io_inner_grant_valid.values[0] && dut.L2Unit__io_inner_grant_ready.values[0])
            {
                std::cout << "inner grant type: " << dut.L2Unit__io_inner_grant_bits_is_builtin_type.values[0] << " " << dut.L2Unit__io_inner_grant_bits_g_type.values[0] << "\n";
                }
