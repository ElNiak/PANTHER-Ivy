...
```
    ...
    action add_def(s:expr,st:flatst,is_global:bool) returns (st:flatst) = {
        st.defining := true;
```
...
```
    ...
    object enumspec = { ...
        ...
        action defd(s:this,st:flatst,id:ident) returns (st:flatst) = {
            ...
            while idx < s.constructors.end {
                st := add_def(s.constructors.value(idx),st,false);
                idx := idx.next;
            }
        }
    }
    ...
    object structspec = { ...
		...
		st.defs := st.defs.set(lhs.get_name.prefix(id),true);
                idx := idx.next;
            }
        }
        action auto_flat_spec(s:this,st:flatst,ty:expr) returns (st:flatst) = {
		...
		st.root := old_root;
                ...
                var dstr := vardc.make(typing,true,e.get_ann);
...
```
	...
	action flat_int(s:this,st:flatst) returns (res:this,st:flatst) = {
	    ...
	    st := auto_defd(s.typing,st);
            st := add_def(get_typed_symbol(s.typing),st,true);
        }
    }
