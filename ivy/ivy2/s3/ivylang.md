...
```
    ...
    object pragmast = {
        ...
        action make (prg:expr,ann:annot) returns (res:stmt) = {
            var s:this;
            s.prg := prg;
            s.ann := ann;
            res := s;
    ...
    object stmt = { ...
        ...
        action parse_stmt_term(st : pstate) returns(st : pstate) = {
            if st.ok & st.tok = ";" {
                st := st.consume;
            } else {
                st.ok := st.ok & st.tok = "}";  # allow to omit final semicolon
            }
        }
        ...
        action parse_lang_stmt(st : pstate, prio:priority) returns(st : pstate, res:stmt) = {
            if st.ok & st.tok = "@" {
                st := st.consume;
                var s : pragmast;
                (st,s.prg) := expr.parse(st,0);
                st := parse_stmt_term(st);
                res := s;
            } else {
                var s : asgn;
                (st,s.ann) := st.get_ann;
