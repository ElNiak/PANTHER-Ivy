...
object char = {
    ...
    function is_hex(X:char) = is_digit(X:char) | (65 <= X & X < 71) | (97 <= X & X < 103)
    function is_capital(X:char) =  (65 <= X & X < 91)
    function downcase(X:char) = (X+32) if is_capital(X:char) else X
}
...
module chars_reader(pred) = {
    action read(st:pstate) returns (st:pstate) = {
        if st.ok & st.p < st.b.end & pred(st.b.value(st.p)) {
            while st.ok & st.p < st.b.end & pred(st.b.value(st.p)) {
                st.tok := st.tok.append(st.b.value(st.p));
	        st.p := st.p.next;
            }
        } else {
            st.ok := false;
...
instance decimals : chars_reader(char.is_digit)
instance hexes : chars_reader(char.is_hex)
...
```
action read_numeric_literal(st:pstate) returns (st:pstate) = {
    st.tok := "";
    if st.p.next < st.b.end & st.b.value(st.p) = 48 & st.b.value(st.p.next).downcase = 120 {
        st.tok := st.tok.append(48);
        st.tok := st.tok.append(120);
	st.p := st.p.next.next;
        st := hexes.read(st);
    } else {
        st := decimals.read(st);
        if st.ok & st.p < st.b.end & st.b.value(st.p) = 46 {
            st.tok := st.tok.append(46);
            st.p := st.p.next;
            st := decimals.read(st);
            if st.ok & st.p < st.b.end & st.b.value(st.p).downcase = 101 {
                st.tok := st.tok.append(101);
                st.p := st.p.next;
                if st.p < st.b.end & (st.b.value(st.p) = 45 | st.b.value(st.p) = 43) {
                    st.tok := st.tok.append(st.b.value(st.p));
                    st.p := st.p.next;
                };
                st := decimals.read(st);
...
```
    ...
    } else if st.b.value(st.p).is_digit {
        st := read_numeric_literal(st);
    } else {
        var last : char := 32;
        while st.p < st.b.end & ~st.b.value(st.p).is_white
