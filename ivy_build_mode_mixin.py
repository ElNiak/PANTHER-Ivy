...
class IvyBuildModeMixin:
    ...
    For local development, this mixin allows setting and retrieving build mode
    configurations that control compilation flags and Z3 build settings.

    Provides four build modes while preserving original Shadow compatibility:
    ...
    Note: This mixin expects the parent class to provide a `logger` attribute
    and optionally `service_config_to_test` or `config` for build mode retrieval.
    """
    ...
    def _copy_z3_library(self, build_dir: str, project_root: str) -> None:
        ...
        CMake installs libraries to {CMAKE_INSTALL_PREFIX}/lib/, so we look there.
        We also check the build directory as a fallback for non-install builds.
        """
        import shutil
        ...
        # Primary location: CMake install prefix lib directory
        # (CMAKE_INSTALL_PREFIX is set to project_root in _build_z3_cmake)
        install_lib_path = os.path.join(project_root, 'lib', lib_name)
        # Fallback: build directory (for non-install builds)
        build_lib_path = os.path.join(build_dir, lib_name)

        # Try install location first, then build directory
        if os.path.exists(install_lib_path):
            src_path = install_lib_path
        elif os.path.exists(build_lib_path):
            src_path = build_lib_path
        else:
            self.logger.warning(
                f"Z3 library not found at {install_lib_path} or {build_lib_path}"
            )
            return
