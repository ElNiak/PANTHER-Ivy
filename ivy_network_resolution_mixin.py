...
class IvyNetworkResolutionMixin:
    ...
    def resolve_service_target_placeholders(
        ...
        for key, value in template_context.items():
            if isinstance(value, str):
                resolved_value = self._resolve_placeholders_in_string(
                    value, role_to_service_mapping
                )
    ...
    def _get_service_targets(self) -> List[str]:
        ...
        if hasattr(self, "service_targets") and self.service_targets:
            ...
            if isinstance(self.service_targets, list):
                ...
                self.logger.warning(
                    f"Unexpected type for service_targets: {type(self.service_targets)}"
                )
        ...
        if hasattr(self, "_original_service_config"):
            ...
            self.logger.debug(
                f"Using _original_service_config targets: {config_targets}"
            )
            if config_targets:
                ...
                if isinstance(config_targets, list):
                    ...
                    self.logger.warning(
                        f"Unexpected type for _original_service_config.targets: {type(config_targets)}"
                    )
            ...
            if experiment_services:
                for service_name, service_config in experiment_services.items():
                    ...
                    if service_type not in [
                        "tester",
                        "testers",
                    ] and service_name != getattr(self, "service_name", ""):
    ...
    def _build_role_to_service_mapping(
        self, service_targets: List[str]
    ) -> Dict[str, str]:
        """
        ...
        self.logger.debug(
            f"Building role mapping for ivy role: {ivy_role} with primary target: {primary_target}"
        )
    ...
    def _resolve_placeholders_in_string(
        self, text: str, role_mapping: Dict[str, str]
    ) -> str:
        """
        ...
        def replace_placeholder(match):
            ...
            self.logger.debug(
                f"Resolved placeholder: {match.group(0)} -> {resolved_placeholder}"
            )
