...
class PathTemplateResolver(LoggerMixin):
    ...
    def _setup_default_variables(self):
        ...
        defaults = {
            ...
            "PANTHER_IVY_BASE_DIR": os.getenv(
                "PANTHER_IVY_BASE_DIR", f"{source_dir}/panther_ivy"
            ),
            "PANTHER_IVY_INSTALL_DIR": os.getenv(
                "PANTHER_IVY_INSTALL_DIR", "/opt/panther_ivy"
            ),
            "IVY_PROTOCOL_BASE": os.getenv(
    ...
    def resolve_template(
        self, template: str, context: Optional[Dict[str, str]] = None
    ) -> str:
        """
    ...
    def resolve_template_dict(
        self, template_dict: Dict[str, Any], context: Optional[Dict[str, str]] = None
    ) -> Dict[str, Any]:
        """
        ...
        for key, value in template_dict.items():
            if isinstance(value, str):
                ...
                resolved[key] = [
                    self.resolve_template(item, context)
                    if isinstance(item, str)
                    else self.resolve_template_dict(item, context)
    ...
    def create_architecture_context(
        self, use_apt_protocols: bool = False, protocol_name: str = "quic"
    ) -> Dict[str, str]:
        """
        ...
        if use_apt_protocols:
            ...
            base_dir = (
                f"{project_root}/protocol-testing/apt/apt_protocols/{protocol_name}"
            )
            context |= {
            ...
            self.logger.debug(
                f"Using individual protocol architecture for {protocol_name}"
            )
...
def resolve_path_template(
    template: str, context: Optional[Dict[str, str]] = None
) -> str:
    """
...
def create_apt_context(protocol_name: str = "quic") -> Dict[str, str]:
    ...
    return get_global_resolver().create_architecture_context(
        use_apt_protocols=True, protocol_name=protocol_name
    )
...
def create_individual_context(protocol_name: str = "quic") -> Dict[str, str]:
    ...
    return get_global_resolver().create_architecture_context(
        use_apt_protocols=False, protocol_name=protocol_name
    )
