...
```
            ...
            individual dcil_exported : bool

            individual number_of_connection_from(S:ip.addr) : stream_pos
            ...
            import action receiving_time(s:seconds)
...
```
                ...
                while idx < pkts.end {
                    call receiving_time(time_api.c_timer.now_sec);
                    var ppkt := pkts.value(idx);
                    var tls_id       := src_tls_id(dst); # todo multiple
                    ...
                    if ~nat_configured |( src.addr = ep_client.addr & src.port ~= ep_client.port) {
                        nat_configured := true;
                        ep_client      := src; # Like initial connection from client

                        client.set_ep(src);
                        mim_agent.ep_target_client.addr := src.addr + 2;
                        mim_agent.ep_target_client.port := src.port;
                        ...
                        number_of_connection_from(dst.addr) := number_of_connection_from(dst.addr) + 1;
                        call show_number_of_connection(src.addr,number_of_connection_from(src.addr));
                        call show_number_of_connection(dst.addr,number_of_connection_from(dst.addr));

                        if number_of_connection_from(dst.addr) = 2 {
                            call mim_agent.disable_ip_forward;
...
```
                            call tls_start_eavedrop_client_initial_request(src,dst,ppkt);
...
if bvand(ppkt.value(0),0xf0) = 0xc0 {
    number_of_connection_from(src.addr) := number_of_connection_from(src.addr) + 1;
    call show_number_of_connection_debug_event(src.addr,number_of_connection_from(src.addr));
}
...
```
                            ...
                            if is_long {
                                if is_random_padding_packet(ppkt) {
                                    ...
                                    if already_forwarded(src,dst,rppkt) {
                                        call arp_duplicated_encrypted_random_quic_packet_debug_event(src,dst,rppkt);
                                    } else {
                                        call recv_random_padding_or_short_packet_event(src,dst,rppkt.payload.end);
                                        to_forward := true;
                                        if forward_packets {
                                            call receiving_packet(src,dst,rppkt, packet_number_of_coalesed);
                                            packet_number_of_coalesed := packet_number_of_coalesed + 1;
...
```
                                    var rppkt : packet.encrypted_quic_packet;
                                    if dst = ep_server {
                                        rppkt := quic_encrypted_packet_serdes_client.from_bytes(ppkt);
                                    } else {
                                        rppkt := quic_encrypted_packet_serdes_server.from_bytes(ppkt);
                                    }
                                    conn_seen(rppkt.dst_cid)    := true;
                                    if already_forwarded(src,dst,rppkt) {
                                        call arp_duplicated_encrypted_packet_debug_event(src,dst,rppkt);
                                    } else {
                                        to_forward := true;
                                        call recv_encrypted_packet(host,src,dst,rppkt.head_byte, rppkt.src_cid,rppkt.dst_cid,rppkt.pversion, rppkt.token,rppkt.seq_num_payload.end);
                                        if forward_packets {
                                            call receiving_packet(src,dst,rppkt, packet_number_of_coalesed);
                                            packet_number_of_coalesed := packet_number_of_coalesed + 1;
                                        };
...
```
                                    to_forward := true;
                                    var rppkt : packet.encrypted_short_quic_packet;
                                    if dst = ep_server {
                                        rppkt := quic_encrypted_short_packet_serdes_client.from_bytes(ppkt);
                                    } else {
                                        rppkt := quic_encrypted_short_packet_serdes_server.from_bytes(ppkt);
                                    }
                                    if ~conn_seen(rppkt.dst_cid) {
                                        var rrppkt := quic_random_padding_encrypted_packet_serdes.from_bytes(ppkt);
                                        call recv_random_padding_or_short_packet_event(src,dst,rrppkt.payload.end);
                                        to_forward := true;
                                        if forward_packets {
                                            call receiving_packet(src,dst,rrppkt, packet_number_of_coalesed);
                                            packet_number_of_coalesed := packet_number_of_coalesed + 1;
                                        };
                                    } else
                                    if already_forwarded(src,dst,rppkt) {
                                        ...
                                        call recv_encrypted_short_packet(src,dst,rppkt.head_byte,rppkt.dst_cid,rppkt.payload.end);
                                        if forward_packets {
                                            call receiving_packet(src,dst,rppkt, packet_number_of_coalesed);
                                            packet_number_of_coalesed := packet_number_of_coalesed + 1;
                                        };
...
```
                    ...
                    call tls_api.upper.eavesdrop_client(tls_id);
                    call show_set_initial_keys(ikm, tls_id, dcid);
            ...
            action tls_start_eavedrop_client_initial_request(src:ip.endpoint,dst:ip.endpoint,spkt:stream_data) = {
```
...
```
                    ...
                    call tls_api.upper.eavesdrop_server(3);
                }
...
```
                ...
                if server_initial_response {
                    call show_set_initial_keys(ikm, other_tls_id,dcid);
                    call tls_api.upper.set_initial_keys(other_tls_id,salt,ikm); ## WTF TODO error id tls_api.upper.set_initial_keys(id,salt,ikm);
                } else {
                    call show_set_initial_keys(ikm, 3, dcid);
                    call tls_api.upper.set_initial_keys(3,salt,ikm);
                }
...
```
            ...
            action configure_NAT_attack_bidir(src:ip.addr, dst:ip.addr) = {
                <<<
                    //  very important for modifying the packet
                    struct in_addr src_addr;
                    src_addr.s_addr = htonl(src); // Convert to network byte order
            ...
            action disable_ip_forward = {
                <<<
                    //  very important for modifying the packet

                    std::string command = "sysctl -w net.ipv4.ip_forward=0";
                    int n = command.length();
                    char command_array[n + 1];
                    strcpy(command_array, command.c_str());
                    int cr = system(command_array);
                    ...
                    command = "sysctl -w net.ipv4.conf.ip_nonlocal_bind=0";
                    n = command.length();
                    command_array[n + 1];
                    strcpy(command_array, command.c_str());
                    cr = system(command_array);
            ...
            action enable_ip_forward = {
                <<<
                    //  very important for modifying the packet

                    std::string command = "sysctl -w net.ipv4.ip_forward=1";
                    int n = command.length();
                    char command_array[n + 1];
                    strcpy(command_array, command.c_str());
                    int cr = system(command_array);

                    command = "sysctl -w net.ipv4.conf.all.send_redirects=1";
                    n = command.length();
                    command_array[n + 1];
                    strcpy(command_array, command.c_str());
                    cr = system(command_array);

                    command = "sysctl -w net.ipv4.conf.default.send_redirects=1";
                    n = command.length();
                    command_array[n + 1];
                    strcpy(command_array, command.c_str());
                    cr = system(command_array);

                    command = "sysctl -w net.ipv4.conf.ip_nonlocal_bind=1";
                    n = command.length();
                    command_array[n + 1];
                    strcpy(command_array, command.c_str());
                    cr = system(command_array);
            ...
            import action show_number_of_connection(src:ip.addr,s:stream_pos)
            ...
            import action recv_encrypted_short_packet(src:ip.endpoint, dst:ip.endpoint, pkt_head_byte:stream_pos, pkt_scid: cid, pkt_len: stream_pos)
            import action recv_random_padding_or_short_packet_event(src:ip.endpoint, dst:ip.endpoint, pkt:stream_pos)
            ...
            import action arp_duplicated_encrypted_random_quic_packet_debug_event(src:ip.endpoint, dst:ip.endpoint, pkt:packet.random_padding_encrypted_quic_packet)
            import action recv_packet(host:endpoint_id,src:ip.endpoint,dst:ip.endpoint,pkt:packet.quic_packet)
            import action recv_encrypted_packet(host:endpoint_id,src:ip.endpoint,dst:ip.endpoint, pkt_head_byte:stream_pos, pkt_scid: cid, pkt_dcid:cid, pkt_version: version, pkt_token:stream_data, pkt_len:stream_pos)
        }
...
```
                ...
                while idx < pkts.end {
                    ...
                    var h := net_prot.get_header_info(ppkt,false);
                    ...
                    var tls_id := cid_map_tls_id(h.dcid); #src_tls_id(dst); # todo multiple
                    var dst    := cid_map_ep(h.dcid); #dst_tls_id(dst); # todo multiple
                    if is_long & ~conn_seen(h.scid) & conn_requested(dst,src,h.dcid) { # & conn_requested(dst,src,h.scid)
                        cid_map_tls_id(h.scid)      := server.tls_id;
                        cid_map_ep(h.scid)          := src;
                        cid_map_sock(h.scid)        := s;
                        call show_update_(h.scid,server.tls_id,src);
```
cid_map_ep(dcid)          := dst;
```
                    };
                    call show_recv_debug_ev(src,dst,h.scid,h.dcid,tls_id);
...
```
                            call on_datagram_received(dst, h.dcid, ppkt);
                            if res.ok {
...
```
                                        ...
                                        if pkt.payload.end = 0{ # picoquic send random padding
                                            call padding_packet_event(host,src,dst,ppkt);
                                        } else {
                                            if verify_incoming_packet {
...
```
            ...
            import action show_recv_debug_ev(src:ip.endpoint,dst:ip.endpoint,scid:cid,dcid:cid,tls_id:tls_api.id)
            import action show_update_(cid:cid,tls_id:tls_api.id,ep:ip.endpoint)
