...
action socket_endpoint(host:endpoint_id,s:quic_net.socket) returns (src:ip.endpoint) = {
    src := client.ep if host = endpoint_id.client else (client_vn if version_negociated else client_alt)
}
...
action dst_is_generated(dst:ip.endpoint) returns (res:bool) = {
    res := dst = client.ep | dst = client_alt | dst = client_vn
}
...
```
before quic_frame.crypto.handle_maliciously(f:quic_frame.crypto,scid:cid,dcid:cid,e:quic_packet_type,seq_num:pkt_num, src:ip.endpoint) {
    if _generating {
        require scid = the_cid;
        require f.length > 0;
...
```
before quic_frame.ack.handle_maliciously(f:quic_frame.ack,scid:cid,dcid:cid,e:quic_packet_type,seq_num:pkt_num, src:ip.endpoint) {
    if _generating {
        require scid = the_cid;
        force_new_ack := true;
```
...
```
before quic_frame.rst_stream.handle_maliciously(f:quic_frame.rst_stream,scid:cid,dcid:cid,e:quic_packet_type,seq_num:pkt_num, src:ip.endpoint) {
    if _generating {
        require scid = the_cid;
        require f.id = 4;
    }
...
before quic_frame.new_token.handle_maliciously(f:quic_frame.new_token,scid:cid,dcid:cid,e:quic_packet_type,seq_num:pkt_num, src:ip.endpoint) {
    if _generating {
        require scid = the_cid;
        require f.data.end = 10;
```
call handle_sending_ack(f.largest_acked);
```
    }
...
```
before quic_frame.stop_sending.handle_maliciously(f:quic_frame.stop_sending,scid:cid,dcid:cid,e:quic_packet_type,seq_num:pkt_num, src:ip.endpoint) {
    if _generating {
        require scid = the_cid;
        require f.id = 4;
    }
...
```
before quic_frame.max_streams.handle_maliciously(f:quic_frame.max_streams,scid:cid,dcid:cid,e:quic_packet_type,seq_num:pkt_num, src:ip.endpoint) {
    if _generating {
        require scid = the_cid;
    }
}
...
before quic_frame.retire_connection_id.handle_maliciously(f:quic_frame.retire_connection_id,scid:cid,dcid:cid,e:quic_packet_type,seq_num:pkt_num, src:ip.endpoint) {
    if _generating {
        require scid = the_cid;
    }
}
...
```
before quic_frame.connection_close.handle_maliciously(f:quic_frame.connection_close,scid:cid,dcid:cid,e:quic_packet_type,seq_num:pkt_num, src:ip.endpoint) {
    if _generating {
        require conn_total_data(the_cid) > 1000; # tricks
        require scid = the_cid;
...
```
before quic_frame.application_close.handle_maliciously(f:quic_frame.application_close,scid:cid,dcid:cid,e:quic_packet_type,seq_num:pkt_num, src:ip.endpoint) {
    if _generating {
        require scid = the_cid;
    } else {
        require is_no_error;
...
```
before quic_frame.max_stream_data.handle_maliciously(f:quic_frame.max_stream_data,scid:cid,dcid:cid,e:quic_packet_type,seq_num:pkt_num, src:ip.endpoint) {
    if _generating {
        require scid = the_cid;
...
```
before quic_frame.stream_data_blocked.handle_maliciously(f:quic_frame.stream_data_blocked,scid:cid,dcid:cid,e:quic_packet_type,seq_num:pkt_num, src:ip.endpoint) {
    if _generating {
        require scid = the_cid;
        require f.id = 4;
    }
...
```
before quic_frame.max_data.handle_maliciously(f:quic_frame.max_data,scid:cid,dcid:cid,e:quic_packet_type,seq_num:pkt_num, src:ip.endpoint) {
    if _generating {
        require scid = the_cid;
        require f.pos = 4000;
    }
