...
This creates the sockets and also a TLS instance to be used by ` ep_map_cid(src)`.
...
action dst_is_generated_tls(dst:ip.endpoint) returns (res:bool) = {
    res := dst ~= server.ep; #dst = client.ep | dst = client_alt | dst = client_vn
}
...
before quic_frame.stream.handle(f:quic_frame.stream,scid:cid,dcid:cid,e:quic_packet_type,seq_num:pkt_num, src:ip.endpoint) {
    if _generating {
```
require exists I. I < the_cid_used.end  & the_cid_used.value(I) = scid & dcid = nonce_cid(scid);
...
```
require dcid = nonce_cid(ep_map_cid(src));
...
before quic_frame.connection_close.handle(f:quic_frame.connection_close,scid:cid,dcid:cid,e:quic_packet_type,seq_num:pkt_num, src:ip.endpoint) {
    if _generating {
        require conn_total_data(ep_map_cid(src)) > 1000; # tricks
```
require exists I. I < the_cid_used.end  & the_cid_used.value(I) = scid & dcid = nonce_cid(scid);
require scid = the_cid;
require src.addr = client.ep.addr;
