...
after init {
    ...
    sock := quic_net.open(endpoint_id.server,client_server.ep_server);
}
...
action endpoint_to_pid(src:ip.endpoint) returns (pid:endpoint_id) = {
    if src = client_server.ep_client {
        pid := endpoint_id.client;
    } else if src = client_server.ep_server {
...
action socket_endpoint(host:endpoint_id,s:quic_net.socket) returns (src:ip.endpoint) = {
    src := client_server.ep_client if host = endpoint_id.client else (client_server.ep_server if host = endpoint_id.client else client_alt)
}
...
action endpoint_to_socket(src:ip.endpoint) returns (socket:quic_net.socket) = {
    if src = client_server.ep_client {
        socket := sock_alt;
    } else if client_server.ep_server {
        socket := sock;
    }
}
action tls_id_to_src(tls_id:tls_api.id) returns (src:ip.endpoint) = {
    src := client_server.ep_client if tls_id = client.tls_id & ~version_negociated else (client_vn if tls_id = client.tls_id & version_negociated else client_server.ep_server);
}
...
action tls_id_to_dst(tls_id:tls_api.id) returns (dst:ip.endpoint) = {
    dst := client_server.ep_server if tls_id = client.tls_id else client_server.ep_client;
}
...
action dst_is_generated(dst:ip.endpoint) returns (res:bool) = {
    res := dst = client_server.ep_client | dst = client_alt | dst = client_vn
}
...
action src_tls_id(src:ip.endpoint) returns (tls_id:tls_api.id) = {
    tls_id := client.tls_id if src = client_server.ep_client
...
only one migration
```
                    require src = client_server.ep_client | src = client_alt;
                } else {
                    require src = client_alt;
                }
            } else {
                require src = client_server.ep_client;
            }
        };
...
```
before quic_frame.crypto.handle(f:quic_frame.crypto,scid:cid,dcid:cid,e:quic_packet_type,seq_num:pkt_num, src:ip.endpoint) {
    if _generating {
        require scid = the_cid;
        require f.length > 0;
...
```
before quic_frame.rst_stream.handle(f:quic_frame.rst_stream,scid:cid,dcid:cid,e:quic_packet_type,seq_num:pkt_num, src:ip.endpoint) {
    if _generating {
        require scid = the_cid;
        require f.id = 4;
    }
...
before quic_frame.new_token.handle(f:quic_frame.new_token,scid:cid,dcid:cid,e:quic_packet_type,seq_num:pkt_num, src:ip.endpoint) {
    if _generating {
        require scid = the_cid;
```
call handle_sending_ack(f.largest_acked);
...
```
before quic_frame.stop_sending.handle(f:quic_frame.stop_sending,scid:cid,dcid:cid,e:quic_packet_type,seq_num:pkt_num, src:ip.endpoint) {
    if _generating {
        require scid = the_cid;
        require f.id = 4;
    }
...
before quic_frame.new_connection_id.handle(f:quic_frame.new_connection_id,scid:cid,dcid:cid,e:quic_packet_type,seq_num:pkt_num, src:ip.endpoint) {
    if _generating {
        require scid = the_cid;
    }
}
...
```
before quic_frame.max_stream_data.handle(f:quic_frame.max_stream_data,scid:cid,dcid:cid,e:quic_packet_type,seq_num:pkt_num, src:ip.endpoint) {
    if _generating {
        require scid = the_cid;
...
```
before quic_frame.stream_data_blocked.handle(f:quic_frame.stream_data_blocked,scid:cid,dcid:cid,e:quic_packet_type,seq_num:pkt_num, src:ip.endpoint) {
    if _generating {
        require scid = the_cid;
        require f.id = 4;
    }
...
around client_send_event {
    ...
    require dst = client_server.ep_server;
    if ~zero_rtt_server_test | zero_rtt_sent {
        require connected(the_cid) & dcid = connected_to(the_cid);
    } else {
