...
```
            ...
            action behavior(host:endpoint_id, s:quic_net.socket, src:ip.endpoint, pkts:net_prot.arr) = {
                ...
                while idx < pkts.end {
                    ...
                    var dst    := socket_endpoint(host,s);#dst_tls_id(dst); # todo multiple

                    var tls_id := src_tls_id(dst); #src_tls_id(dst); # todo multiple
```
...
```
            ...
            action behavior(host:endpoint_id, s:quic_net.socket, src:ip.endpoint, pkts:net_prot.arr) = {
                ...
                while idx < pkts.end {
                    ...
                    var dst := server.ep; #dst_tls_id(dst); # todo multiple
...
```
            ...
            import action show_recv_debug_ev(src:ip.endpoint,dst:ip.endpoint,scid:cid,dcid:cid,tls_id:tls_api.id)
            import action show_update_(cid:cid,tls_id:tls_api.id,ep:ip.endpoint)
            import action show_last_datagram_size(s:stream_pos)
        }
    }
}
