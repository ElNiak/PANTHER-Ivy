...
action send_ack_eliciting_handshake_packet(src:ip.endpoint,dst:ip.endpoint,pkt:packet.quic_packet) = {}

action send_ack_eliciting_initial_packet(src:ip.endpoint,dst:ip.endpoint,pkt:packet.quic_packet) = {}

action send_ack_eliciting_application_packet(src:ip.endpoint,dst:ip.endpoint,pkt:packet.quic_packet) = {}


```
action packet_event_multiple_src(src1:ip.endpoint,src2:ip.endpoint,dst:ip.endpoint,pkt:packet.quic_packet) = {}
...
- For each aid C, `draining_pkt_sent(C)` is true if the single
  ...
  This one packet must contain a connection_close quic_frame with error code 0.
...
- The predicate `connected(C)` indicates that the peer of
  ...
  server produces a server hello quic_frame.
...
- The relation `pkt_has_close(C,L,N)` is true if packet number `N` sent
  by aid `C` contained a CONNECTION_CLOSE quic_frame.
...
function is_retransmitted(P:quic_frame.arr) : pkt_num
...
import action show_queued_frames_debug_event(scid:cid,queued_frames:quic_frame.arr)
```
call show_connection_information(scid,dcid, connected(dcid), connected_to(dcid), nonce_cid(scid), pkt.long);

```
import action show_connection_information(scid:cid,dcid:cid,connected:bool,connected_to:cid,nonce_cid:cid,src:ip.endpoint, dst:ip.endpoint)
around packet_event(src:ip.endpoint,dst:ip.endpoint,pkt:packet.quic_packet) {
```
...
```
    ...
    call show_connection_information(scid,dcid, connected(dcid), connected_to(dcid), nonce_cid(scid), src,dst);

    if retry_sent(dcid) & ~_generating & ~zero_length_token & pkt.ptype = quic_packet_type.initial {
        require dcid = client_initial_rcid;
    } else if retry_recv(scid) & _generating & ~zero_length_token & pkt.ptype = quic_packet_type.initial {
        require dcid = client_initial_rcid;
    };
...
TODO puicoquic ping in application data
if _generating  {
```
...
```
    ...
    new_connection_id_frame_pending(scid) := false;
```
Hamid
```
}

import action show_retransmit_lost_packet(paylo:quic_frame.arr)
...
around send_ack_eliciting_handshake_packet(src:ip.endpoint,dst:ip.endpoint,pkt:packet.quic_packet) {
```
is_not_sleeping := time_api.c_timer.is_sleep_fake_timeout;
...
around send_ack_eliciting_application_packet(src:ip.endpoint,dst:ip.endpoint,pkt:packet.quic_packet) {
```
is_not_sleeping := time_api.c_timer.is_sleep_fake_timeout;
require _generating -> is_not_sleeping;
require (_generating  -> is_not_sleeping);
...
around send_ack_eliciting_initial_packet(src:ip.endpoint,dst:ip.endpoint,pkt:packet.quic_packet) {
```
is_not_sleeping := time_api.c_timer.is_sleep_fake_timeout;
require _generating -> is_not_sleeping;
require (_generating  -> is_not_sleeping);
...
import action show_is_retransmitted_debug_event(p:stream_pos, c_time:pkt_num)
...
import action show_current_idle_timeout_debug_event(e:microseconds)
