...
```
        ...
        action encrypt_quic_no_checksum(c:tls_id,seq:pkt_num,pkt:stream_data) returns (pkt:stream_data)
...
packet, which is needed to decrypt_quic the payload. This means that
...
```
        ...
        implement encrypt_quic_no_checksum {
...
always two bytes.
if h.hdr_long {
    var new_len := to_var_int_16(h.payload_length);
    pkt := pkt.set(pnum_pos-2,new_len.value(0));
    pkt := pkt.set(pnum_pos-1,new_len.value(1));
};
...
which case the packet won't decrypt_quic.
...
```
        action get_pnum_len_b(pkt:stream_data) returns (pnum_len:stream_pos) = {
            ...
            std::cerr <<  "HACK: export get_pnum_len_b"  << pnum_len << "\n";
            char str_d[20];
            sprintf(str_d,"%llu",pnum_len);
            if(setenv("PNUM_LEN",str_d,true) != 0)
...
<<< impl
    ...
    public:
        ...
        virtual void  set(int128_t res) {
            //std::cerr << "quic_prot_ser res size " << this->res.size() << "\n";
            setn(res,1);
        }
    };
...
<<< impl
    ...
    class `quic_prot_deser` : public ivy_binary_deser_128 {
        int data_remaining;
        int total_data_remaining;
        int level;
    public:
        quic_prot_deser(const std::vector<char> &inp) : ivy_binary_deser_128(inp),level(0) {
            std::cerr << "quic_prot_deser size " << inp.size() << "\n";
            total_data_remaining = inp.size();
        }
        ...
        void open_list() {
            std::cerr << "quic_prot_deser imp size " << inp.size() << "\n";
            if (level != 1)
                return;
            if (peek(pos) & 0x80) { // a long packet
                ...
                if (inp[1]+inp[2]+inp[3]+inp[4] == 0) {
                    ...
                    std::cerr << "data_remaining (net_prot) " << data_remaining << "\n";
                    std::cerr << "pyld_len (net_prot) " << pyld_len << "\n";
                    const char * test_type = getenv("TEST_TYPE");
                    if(ptype == 0x00 && pyld_len < 1200 && strcmp(test_type,"mim") == 0) {
                        //coalescing initial packets
                        std::cerr <<  "set env COALESCED_PACKET"  << "\n";
                        char str_d[20];
                        sprintf(str_d,"%s","COALESCED_PACKET");
                        if(setenv("COALESCED_PACKET",str_d,true) != 0)
                            {fprintf(stderr, "setenv failed\n");}
                    }
                    else if(test_type != NULL && strcmp(test_type,"mim") == 0) {
                        std::cerr << "mim test ->"   << std::endl;
                        std::cerr <<  "set env PACKET_SIZE"  << "\n";
                        char str_d[20];
                        sprintf(str_d,"%u",data_remaining);
                        if(setenv("PACKET_SIZE",str_d,true) != 0)
                           { fprintf(stderr, "setenv failed\n");}
                    }
                }
                total_data_remaining = total_data_remaining - data_remaining;
                std::cerr << "total_data_remaining (net_prot) " << total_data_remaining << "\n";
                //std::cerr << "pkt len: " << data_remaining << " type: " << (peek(pos)-pos) << " len len: " << len << " pyld_len: " << pyld_len << " pnum_pos: " << pnum_pos << "\n";
            }
            else {
                std::cerr << "quic_prot_deser a short packet\n";
                data_remaining = total_data_remaining; //inp.size(); //!more(1) should stop automatically
                std::cerr << "data_remaining (net_prot) " << data_remaining << "\n";
            }
        }
