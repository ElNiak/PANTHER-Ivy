...
```
    ...
    var hs := res.value;
    call tls.handshake_data_event(src,dst,avail.segment(0,res.pos),dcid);
...
around tls_recv_event {
    ...
    require dst.addr = client.ep.addr;
    require used_client_ports(dst.port);
    require dst.protocol = ip.udp;
    require dst.interface = ip.veth_ivy;
    require ep_present(dst) & dcid = ep_map_cid(dst);
