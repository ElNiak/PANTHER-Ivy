...
```
before quic_frame.max_streams_bidi.handle(f:quic_frame.max_streams_bidi,scid:cid,dcid:cid,e:quic_packet_type,seq_num:pkt_num, src:ip.endpoint) {
    if _generating {
...
before quic_frame.connection_close.handle(f:quic_frame.connection_close,scid:cid,dcid:cid,e:quic_packet_type,seq_num:pkt_num, src:ip.endpoint) {
    if _generating {
```
require conn_total_data(the_cid) > 10; # tricks
require last_pkt_num(the_cid,quic_packet_type.one_rtt) > 2;
```
        require client.ep.addr ~= 0;
```
...
before quic_frame.application_close.handle(f:quic_frame.application_close,scid:cid,dcid:cid,e:quic_packet_type,seq_num:pkt_num, src:ip.endpoint) {
    if _generating {
```
require conn_total_data(the_cid) > 3; # tricks (4bytes is one stream quic_frame data)
```
        require last_pkt_num(the_cid,quic_packet_type.one_rtt) > 2;
        require client.ep.addr ~= 0;
        require tls_handshake_finished;
        ...
        call _finalize;
```
call quic_net.close(endpoint_id.client,sock);
```
    }
}
...
export quic_frame.max_streams_bidi.handle
export quic_frame.connection_close.handle # for quinn no closing co
...
```
export quic_frame.application_close.handle # TODO need to be close for picoquic, but not for quant
```
}
