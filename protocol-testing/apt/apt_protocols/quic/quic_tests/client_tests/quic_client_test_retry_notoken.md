...
before packet_event_retry(src:ip.endpoint,dst:ip.endpoint,pkt:packet.quic_packet_retry) {
    if _generating {
...
```
        ...
        var rt := net_prot.retry_integrity_tag(client_initial_dcil,client_initial_dcid,
                                           initial_version,
