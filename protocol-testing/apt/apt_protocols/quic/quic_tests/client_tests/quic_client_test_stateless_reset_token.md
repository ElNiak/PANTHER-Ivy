...
```
    ...
    var token : stateless_reset_token; # TODO normal senario
    token.data_8 := random_stream_data(0,0xFFFFFFF,16);
    tp.transport_parameters := tp.transport_parameters.append(token);
}
