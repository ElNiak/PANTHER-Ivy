...
after init {
    ...
    sock_alt := quic_net.open(endpoint_id.client_alt,second_client.ep);
    sock_vn := sock; #quic_net.open(endpclientoint_id.client_vn,client_vn);
    client.set_tls_id(1);
...
This is the mirror constraint for the stream quic_frameframeframe protocol. We add
...
ones) so we require that a stream quic_frameframeframe send all of the available
stream data.
...
```
before quic_frameframeframe.streamquic_framequic_framequic_frame.stream,scid:cid,dcid:cid,e:quic_packet_type,seq_num:pkt_num, src:ip.endpoint) {
    if _generating {
...
```
before quic_frameframeframe.cryptoquic_framequic_framequic_frame.crypto,scid:cid,dcid:cid,e:quic_packet_type,seq_num:pkt_num, src:ip.endpoint) {
    if _generating {
        require scid = the_cid | scid = the_cid_alt;
        require f.length > 0;
...
```
before quic_frameframeframe.ackquic_framequic_framequic_frame.ack,scid:cid,dcid:cid,e:quic_packet_type,seq_num:pkt_num, src:ip.endpoint) {
    if _generating {
        require scid = the_cid | scid = the_cid_alt;
        force_new_ack := true;
```
...
```
before quic_frameframeframe.rst_streamquic_framequic_framequic_frame.rst_stream,scid:cid,dcid:cid,e:quic_packet_type,seq_num:pkt_num, src:ip.endpoint) {
    if _generating {
        require scid = the_cid | scid = the_cid_alt;
        require f.id = 4;
    }
...
```
before quic_frameframeframe.stop_sendingquic_framequic_framequic_frame.stop_sending,scid:cid,dcid:cid,e:quic_packet_type,seq_num:pkt_num, src:ip.endpoint) {
    if _generating {
        require scid = the_cid | scid = the_cid_alt;
        require f.id = 4;
    }
...
```
before quic_frameframeframe.max_streamsquic_framequic_framequic_frame.max_streams,scid:cid,dcid:cid,e:quic_packet_type,seq_num:pkt_num, src:ip.endpoint) {
    if _generating {
        require scid = the_cid | scid = the_cid_alt;
    }
}

before quic_frameframeframe.new_connection_idquic_framequic_framequic_frame.new_connection_id,scid:cid,dcid:cid,e:quic_packet_type,seq_num:pkt_num, src:ip.endpoint) {
    if _generating {
        require scid = the_cid | scid = the_cid_alt;
    }
}
...
before quic_frameframeframe.retire_connection_idquic_framequic_framequic_frame.retire_connection_id,scid:cid,dcid:cid,e:quic_packet_type,seq_num:pkt_num, src:ip.endpoint) {
    if _generating {
        require scid = the_cid | scid = the_cid_alt;
    }
}
...
```
before quic_frameframeframe.max_stream_dataquic_framequic_framequic_frame.max_stream_data,scid:cid,dcid:cid,e:quic_packet_type,seq_num:pkt_num, src:ip.endpoint) {
    if _generating {
        require scid = the_cid | scid = the_cid_alt;
...
We restrict the quic_frameframeframe id.

```
before quic_frameframeframe.stream_data_blockedquic_framequic_framequic_frame.stream_data_blocked,scid:cid,dcid:cid,e:quic_packet_type,seq_num:pkt_num, src:ip.endpoint) {
    if _generating {
        require scid = the_cid | scid = the_cid_alt;
        require f.id = 4;
    }
...
```
before quic_frameframeframe.max_dataquic_framequic_framequic_frame.max_data,scid:cid,dcid:cid,e:quic_packet_type,seq_num:pkt_num, src:ip.endpoint) {
    if _generating {
        require scid = the_cid | scid = the_cid_alt;
        require f.pos = 4000;
    }
...
```
attribute quic_frameframeframe.crypto.handle.weight = "5"
attribute quic_frameframeframe.path_response.handle.weight = "5"
