...
```
before quic_frame.ack_frequency.handle(f:quic_frame.ack_frequency,scid:cid,dcid:cid,e:quic_packet_type,seq_num:pkt_num, src:ip.endpoint) {
    if _generating {
...
before quic_frame.immediate_ack.handle(f:quic_frame.immediate_ack,scid:cid,dcid:cid,e:quic_packet_type,seq_num:pkt_num, src:ip.endpoint) {
    if _generating {
        require client.ep.addr ~= 0;
        require scid = the_cid;
...
export quic_frame.ack_frequency.handle
```
export quic_frame.immediate_ack.handle
```
