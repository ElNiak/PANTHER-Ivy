...
include quic_locale
include ivy_quic_two_clients_behavior
...
after init {
    ...
    the_cids := the_cids.append(the_cid);
    the_cids := the_cids.append(the_cid_alt);

    server_cids := server_cids.append(server_cid);
    server_cids := server_cids.append(server_cid_alt);
}
...
TODO     {"quic_transport_parameters", 0xffa5}, //DRAFTversion: 0xffa5 -> TODO test 0x39 - TODO RETRY VULN
...
if src = second_client.ep {
```
                var expected_token := tls_api.upper.get_old_new_token; ## TODO RETRY token ??
                pkt.token := expected_token;
```
} else {
