...
```
before quic_frame.unknown_frame.handle(f:quic_frame.unknown_frame,scid:cid,dcid:cid,e:quic_packet_type,seq_num:pkt_num, src:ip.endpoint) {
    if _generating {
...
export quic_frame.max_stream_data.handle
  ...
  An endpoint MUST treat the receipt of a quic_frame of unknown type as a
  connection error of type FRAME_ENCODING_ERROR.

```
export quic_frame.unknown_frame.handle
attribute quic_frame.unknown_frame.handle.weight = "5"
