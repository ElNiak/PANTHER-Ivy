...
parameter client_port_vn : ip.port = 4986
...
after init {
    ...
    sock_vn := quic_net.open(endpoint_id.client_vn,client_vn);
    client.set_tls_id(1);
...
```
    var vn : version_information; #TODO update v29
    vn.chosen_version := 0x00000001;
    vn.other_version := versions_bv.empty;
    vn.other_version := vn.other_version.append(0xff00001f);
```
vn.other_version := vn.other_version.append(0xff00001c);
```
    tp.transport_parameters := tp.transport_parameters.append(vn);
