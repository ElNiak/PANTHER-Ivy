...
```
    ...
    implementation {
        ...
        if(session_file != NULL) {
            ...
            const char* apt_path = getenv("IS_APT_PATH");
            if (base_dir) {
                pathname2 += "/";
                pathname2 += base_dir;
                pathname2 += "/";
                if (apt_path && strlen(apt_path) > 0) {
                    pathname2 += apt_path;
                    pathname2 += "/";
                }
                pathname2 += "quic/quic_utils/";
            } else {
                pathname2 += "/opt/panther_ivy/apt/apt_protocols/quic/quic_utils/";
            }
            pathname = pathname2 + pathname;
        } else {
            char * session_file2 = getenv("IVY_INCLUDE_PATH");
            pathname = session_file2 + pathname; // Last resort (Python path)
        }
        std::cerr << pathname << std::endl;
		...
		`fildes` = f;
        std::cerr << "File opening: " << `fildes` << std::endl;

	    }
        >>>
        implement write {
        <<<
            std::cerr << "Writing " << d.size() << " bytes to " << `fildes` << std::endl;
        ...
        implement read {
        <<<
            std::cerr << "Reading file: " << `fildes` << std::endl;
            if (::lseek(`fildes`,0,SEEK_SET) < 0) {
                perror("cannot seek to beginning of file");
            }
            std::vector<char> buf;
            ...
            //std::cerr << d << std::endl;
            std::cerr << "Reading " << d.size() << " bytes from file" << std::endl;
            //std::cerr << contents << std::endl;
        >>>
        }
        <<< impl
