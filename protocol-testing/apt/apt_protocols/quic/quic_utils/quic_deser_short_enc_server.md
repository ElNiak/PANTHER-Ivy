...
```
object quic_deser_short_enc_server = {}
...
```
    class `quic_deser_short_enc_server` : public ivy_binary_deser_128 {
    //class `quic_deser_short_enc_server` : public ivy_binary_deser {
        enum {quic_s_init,
              quic_s_type,
              quic_s_version,
	          quic_s_dcil,
    ...
    public:
        quic_deser_short_enc_server(const std::vector<char> &inp) : ivy_binary_deser_128(inp),state(quic_s_init) {
        //quic_deser_short_enc_server(const std::vector<char> &inp) : ivy_binary_deser(inp),state(quic_s_init) {
        ...
        //virtual void  get(long long &res) {
            ...
            case quic_s_init:
            {
                ...
                std::cerr << "quic_deser_short_enc_server res " << res << "\n";
                std::cerr << "inp size " << inp.size() << "\n";
                packet_number_length = (res & 0xc0) >> 6; //(res >> 5) & 0x03;
                packet_number_length += 1;
                std::cerr << "quic_deser_short_enc_server hdr_type " << hdr_type << "\n";
                std::cerr << "quic_deser_short_enc_server packet_number_length " << packet_number_length << "\n";
                std::cerr << "quic_deser_short_enc_server long_format " << long_format << "\n";
            ...
            case quic_s_payload_length:
            {
                ...
                payload_length = res;
                data_remaining = res;
                std::cerr << "quic_deser_short_enc_server payload_length " << payload_length << "\n";
            ...
            break;
            default:
                std::cerr << "quic_deser_short_enc_server 3\n";
                throw deser_err();
            }
        }
        ...
        int get_pkt_num(int128_t &res) {
            ...
            std::cerr << "quic_deser_short_enc_server get_pkt_num test " << res << "\n";
        ...
        virtual int open_tag(const std::vector<std::string> &tags) {
            ...
            std::cerr << "quic_deser_short_enc_server 2\n";
            throw deser_err();
        }
        ...
        virtual bool open_list_elem() {
            ...
            std::cerr << "quic_deser_short_enc_server 1\n";
            throw deser_err();
        }

        void open_list() {
            if (state == quic_s_payload) {
                std::cerr << "quic_deser_short_enc_server open_list quic_s_payload\n";
        ...
        void close_list() {
            ...
            if (state == quic_s_retry_token) {
                std::cerr << "quic_deser_short_enc_server close_list quic_s_retry_token\n";
