...
define QUIC_SER_DESER_HEADER_PATH PANTHER_IVY_BASE_DIR/quic_utils/quic_ser_deser.h
include QUOTEME(QUIC_SER_DESER_HEADER_PATH)
...
```
    ...
    //class `quic_ser` : public ivy_binary_ser {
        ...
        int tcount = 0;
        bool done = false;
        bool is_new_connection = false; //TODO for loris attack
    ...
    public:
        ...
        //virtual void  set(long long res) {
            ...
            case quic_s_init:
            {
                ...
                long_format = res != 3;
               /* if(long_format) is_new_connection = true;
                else if(long_format && is_new_connection) is_new_connection = true;
            ...
            break;
            case quic_malicious:
            case quic_malicious_frame_type:
        ...
        virtual void open_tag(int tag, const std::string &) {
            if (state == quic_s_payload) {
                ...
                if (tag == 0) {
                    ...
                    //ivy_binary_deser::getn(ft,1); // could be bigger
                    std::cerr << "quic_ser malicious unknown frame\n";
                    state = quic_malicious_frame_type;
                    sz = 0;
                    frame_type = 0x7b02;
                }
                /*else if (tag == 25) {
                ...
                std::cerr << "quic_ser open_tag " << tag << "\n";
                std::cerr << "quic_ser open_tag " << frame_type << "\n";
                if(sz > 0)
                    setn(frame_type,sz);
                return;
            }
        ...
        void close_list() {
            if (state == quic_s_payload) {
                if (const char* env_p3 = std::getenv("COALESCED_PACKET")) {
                    std::cerr << "ser: COALESCED_PACKET   = " << "\n";
                    unsetenv("COALESCED_PACKET");
                } else {
                    if (const char* env_size = std::getenv("PACKET_SIZE")) {
                        int size = atoi(env_size);
                        std::cerr << "ser: PACKET_SIZE   = " << size << "\n";
                        if (long_format && (hdr_type & 0x30) == 0x00)
                            while (res.size() < size-16) //minus checksum
                                res.push_back(0);
                        unsetenv("PACKET_SIZE");

                    } else {
                        std::cerr << "ser: NOT_COALESCED_PACKET   = " << "\n";
                        if (long_format && (hdr_type & 0x30) == 0x00)
                            while (res.size() < 1200)
                ...
                std::cerr << "ser: PACKET_SIZE   = " << res.size() << "\n";
                std::cerr << "ser: PACKET_SIZE   = " << this->res.size() << "\n";
                if (long_format) {
                    ...
                    std::cerr << "ser: PACKET_SIZE   = " << len << "\n";

                    res[payload_length_pos] = 0x40 | ((len >> 8) & 0x3f);

                    std::ios_base::fmtflags original_flags = std::cerr.flags();
                    std::cerr << "\\x" << std::hex << std::setw(2) << std::setfill('0') << (unsigned int)(unsigned char)res[payload_length_pos];
                    std::cerr.flags(original_flags);  // Restore the original formatting flags
			        std::cerr << std::endl;

                    std::cerr << "ser: PACKET_SIZE   = " << res[payload_length_pos] << "\n";

                    res[payload_length_pos+1] = len & 0xff;

                    original_flags = std::cerr.flags();
                    std::cerr << "\\x" << std::hex << std::setw(2) << std::setfill('0') << (unsigned int)(unsigned char)res[payload_length_pos+1];
                    std::cerr.flags(original_flags);  // Restore the original formatting flags
			        std::cerr << std::endl;

                    std::cerr << "ser: PACKET_SIZE   = " << res[payload_length_pos+1] << "\n";
                    state = quic_s_init;
                }
            }
