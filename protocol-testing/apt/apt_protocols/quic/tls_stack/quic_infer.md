...
import action show_retry_version_negociation(b:bool,bb:bool,bbb:bool)
import action show_nonce_cid_event(scid:cid,dcid:cid)

action infer_tls_events(src:ip.endpoint,dst:ip.endpoint,pkt:packet.quic_packet) = {
```
call infer_tls;
...
```
    if pkt.ptype = quic_packet_type.initial & ~conn_seen(dcid) & ~negocation_of_version(src) &  ~version_negociated {
        nonce_cid(scid) := dcid;
        call show_nonce_cid_event(scid, dcid);
    };

    if pkt.ptype = quic_packet_type.initial & (retry_sent(scid) | version_negociated | negocation_of_version(src)) {
        nonce_cid(scid) := dcid;
        conn_seen(dcid) := false;
        call show_retry_version_negociation(retry_sent(scid),version_negociated,negocation_of_version(src));
```
...
```
    ...
    while idx < pkt.payload.end {
        ...
        if some(cf:quic_frame.crypto) f *> cf {
```
call show_biatch_2(dst);
if  verify_incoming_packet {
    call tls_send_event(src,dst,scid,dcid,cf.data,cf.offset,e,cid_map_tls_id(dcid))
} else {
```
...
```
            ...
            call quic_app_send_event(src,dst,dcid,sf.id,sf.data,sf.offset,sf.fin)
        };
```
if some(pf:quic_frame.padding) f *> pf {
do nothing
} else {
```
...
```
        call f.handle(scid,dcid,e,sq,src);
        idx := idx.next
    }
}
...
action infer_tls_events_vn(src:ip.endpoint,dst:ip.endpoint,pkt:packet.quic_packet_vn) = {
```
Stream frames on id 0 indicate that a `tls_send_event` has occurred.
TODO: ignore duplicate frames.
...
action infer_tls_events_retry(src:ip.endpoint,dst:ip.endpoint,pkt:packet.quic_packet_retry) = {
...
action infer_tls_events_0rtt(src:ip.endpoint,dst:ip.endpoint,pkt:packet.quic_packet_0rtt) = {
...
import action infer_frame(scid:cid,dcid:cid,f:quic_frame)
```
