...
```
before malicious_packet_event_vn(src:ip.endpoint,dst:ip.endpoint,pkt:packet.malicious_quic_packet_vn) { # ,inpkt:packet.quic_packet_vn
    if _generating {
```
require initial_version ~= 0x00000001;
require negocation_of_version(dst);
...
```
        malicious_packet_send := true;
        require (src = mim_agent.ep_client & dst = mim_agent.ep_client); #| (src = server.ep & dst = mim_agent.ep); # Either we manually forge version negociation
        require pkt.dst_cid = client_initial_scid;
        require pkt.src_cid = client_initial_dcid;
        require pkt.pversion = 0x00000000;  #version 0
...
```
        ...
        require pkt = mim_agent.ppkt_to_be_forwarded_to_client(mim_agent.last_packet_forwarded_to(mim_agent.ep_client));
```
...
```
https://github.com/microsoft/ivy/blob/9f3c7ecc0b2383129fdd0953e10890d98d09a82d/protocol-testing/MSV/repstore3_soln.ivy
TODO GUI
