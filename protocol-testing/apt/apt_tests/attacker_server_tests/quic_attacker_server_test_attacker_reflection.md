...
after init {
    sock_target := quic_net.open(endpoint_id.target,mim_agent.ep_target_server.ep);
}

after init {
    sock_attacker := quic_net.open(endpoint_id.malicious_server,attacker.ep);
    sock_alt := quic_net.open(endpoint_id.client_alt,client_alt);
    sock_vn := sock_attacker; # quic_net.open(endpoint_id.client_vn,client_vn);
    attacker.set_tls_id(0);
    server.set_tls_id(0);
    call attacker.configure_NAT_attack_in_spoof_hack(server.ep.port,mim_agent.ep_target_server.ep.port);
    var extns := tls_extensions.empty;
    extns := extns.append(make_transport_parameters);
    call tls_api.upper.create(0,false,extns);  # false means this instance of tls is not a server
}

before malicious_packet_event(src:ip.endpoint,dst:ip.endpoint,pkt:packet.malicious_quic_packet) {
    if _generating {
```
...
chris TODO
```
    require mim_agent.ep_target_server.ppkt_recvd.end > 0;
