...
```
    ...
    mim_agent.ep_target_server.addr := server_addr + 2;
    mim_agent.ep_target_server.port := 4443;
    mim_agent.ep_target_server.protocol := ip.udp;

```
call mim_agent.configure_NAT_attack(mim_agent.ep_target_client.addr,mim_agent.ep_target_server.addr);
call mim_agent.configure_NAT_attack(mim_agent.ep_target_server.addr,mim_agent.ep_target_client.addr);
```
...
object packet = {
    ...
    object encrypted_quic_packet = {
        ...
        before reflect_packet_toward(src:ip.endpoint,dst:ip.endpoint,pkt:packet.encrypted_quic_packet) {
```
require _generating;
```
            if _generating {
...
```
                var interest_pkt := encrypted_quic_packet_received_at_round(src,
...
export packet.encrypted_quic_packet.reflect_packet_toward
...
include stream_data_mim_test_forward
include minip_mim_test_forward
include quic_mim_test_forward

action get_net_from_port(p:ip.port, header_data: stream_data) returns (tp:target_protocol) = {
    if(p = 4430) {
        tp := target_protocol.minip
    } else if (p = 4430) {
        tp := target_protocol.quic
    } else if (p = 80) {
        tp := target_protocol.http
    } else {
        tp := target_protocol.stream_data
    }
}
