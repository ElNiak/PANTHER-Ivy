...
A *id* is a connection identifier. Connection identifiers are
uniquely associated to connections and are not reused. The exception
is that the first id sent by a client in an Initial packet may be
replaced by ther server with a different id, which is used by all
...
The type `id` represents a id as a 64-bit unsigned integer.
...
id, in particular, the first id used by the given application
endpoint. Thus, the type aid is an alias for id. TODO: this will
have to change for endpoints that use zero-length id's, since the
id's in this acse are not unique.
