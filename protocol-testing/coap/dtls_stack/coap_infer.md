...
As an aid in monitoring coap packet streams, this section provides
actions for inferring unobserved ghost events from coap packet
events.

From a coap packet event, infer unobserved TLS events


TODO: as of version 11, each role has its own id, so connections
...
action infer_tls_events(src:ip.endpoint,dst:ip.endpoint,pkt:coap_packet) = {
```
call infer_tls;
...
```
    if pkt.ptype = coap_packet_type.initial & ~conn_seen(dcid) & ~negocation_of_version(src) &  ~version_negociated {
        nonce_cid(scid) := dcid;
    };

    if pkt.ptype = coap_packet_type.initial & (retry_sent(scid) | version_negociated | negocation_of_version(src)) {
        nonce_cid(scid) := dcid;
...
action infer_tls_events_vn(src:ip.endpoint,dst:ip.endpoint,pkt:coap_packet_vn) = {
```
Stream frames on id 0 indicate that a `tls_send_event` has occurred.
TODO: ignore duplicate frames.
...
```
    if pkt.ptype = coap_packet_type.version_negociation  { #& ~conn_seen(dcid)
        nonce_cid(scid) := dcid;
    };

```
var e := coap_packet_type.initial;
...
action infer_tls_events_retry(src:ip.endpoint,dst:ip.endpoint,pkt:coap_packet_retry) = {
...
```
    if pkt.ptype = coap_packet_type.retry {  #& ~conn_seen(dcid)
        nonce_cid(scid) := dcid;
    };
...
action infer_tls_events_0rtt(src:ip.endpoint,dst:ip.endpoint,pkt:coap_packet_0rtt) = {
...
```
    if pkt.ptype = coap_packet_type.zero_rtt {  #& ~conn_seen(dcid)
        nonce_cid(scid) := dcid;
    };

    var e := coap_packet_type.zero_rtt;
...
import action app_server_open_event_1rtt(src:ip.endpoint,dst:ip.endpoint,scid:id,dcid:id)
import action app_server_open_event_0rtt(src:ip.endpoint,dst:ip.endpoint,scid:id,dcid:id)
import action app_server_open_event_retry(src:ip.endpoint,dst:ip.endpoint,scid:id,dcid:id)
import action app_server_open_event_vn(src:ip.endpoint,dst:ip.endpoint,scid:id,dcid:id)
import action infer_frame(scid:id,dcid:id,f:frame)
```
