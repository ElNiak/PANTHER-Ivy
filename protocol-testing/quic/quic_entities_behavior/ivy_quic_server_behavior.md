...
include file
```
collections is included transitively via ivy_quic_shim_server -> quic_shim -> quic_protection
```
...
before frame.handshake_done.handle(f:frame.handshake_done,scid:cid,dcid:cid) {
    if _generating {
        call show_connected(connected(the_cid),the_cid);
        call show_connected(connected(scid),scid);
        call show_connected(connected(dcid),dcid);
        require client.ep.addr ~= 0;
        require scid = the_cid;
