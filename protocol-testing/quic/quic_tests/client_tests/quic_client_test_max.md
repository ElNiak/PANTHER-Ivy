...
```
before frame.max_stream_data.handle(f:frame.max_stream_data,scid:cid,dcid:cid,e:quic_packet_type,seq:pkt_num) {
    if _generating {
        require f.pos = 320 | f.pos = 160;
    }
}

before frame.stream_data_blocked.handle(f:frame.stream_data_blocked,scid:cid,dcid:cid,e:quic_packet_type,seq:pkt_num) {
    if _generating {
        require f.pos > 32;
    }
}

before frame.max_data.handle(f:frame.max_data,scid:cid,dcid:cid,e:quic_packet_type,seq:pkt_num) {
    if _generating {
