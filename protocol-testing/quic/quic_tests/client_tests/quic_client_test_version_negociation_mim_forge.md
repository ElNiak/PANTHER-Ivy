...
```
before forged_packet_event_vn(src:ip.endpoint,dst:ip.endpoint,pkt:forged_quic_packet_vn) {
    if _generating {
```
require initial_version ~= 0x00000001;
require negocation_of_version(dst);
...
```
        forged_packet_send := true;
        require (src = mim_agent.ep_out & dst = mim_client_target.ep); #| (src = server.ep & dst = mim_agent.ep); # Either we manually forge version negociation
        require pkt.dst_cid = client_initial_scid;
        require pkt.src_cid = client_initial_dcid;
        require pkt.pversion = 0x00000000;  #version 0
