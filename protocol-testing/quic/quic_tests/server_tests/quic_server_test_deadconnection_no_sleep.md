...
```
    var is_not_sleeping := time_api.c_timer.is_sleep_fake_timeout;
...
# require (~is_not_sleeping -> ~_generating);
```
    if slept & is_not_sleeping   { # & sleep_start + microseconds_to_milliseconds(max_idle_timeout_used) + 500 < time_api.c_timer.now_millis
```
call _finalize;
```
        if ~_generating {
