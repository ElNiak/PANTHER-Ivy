...
| new_session_ticket   | Client Hello | 4   |
...
etype : extension_type,
```
            max_early_data_size : stream_pos
        }
    }

    object end_of_early_data = {
        variant this of extension = struct {
```
etype : extension_type,
```
...
object pre_shared_key = {
    variant this of extension = struct {
        #etype : extension_type,
        #extensions : vector[offered_psk]
        content : stream_data
    }
}
...
extensions : vector[offered_psk]
```
            identity : stream_data,
            obfuscated_ticket_age : stream_pos
        }
    }

    object pre_shared_key_client = {
        variant this of extension = struct {
```
etype : extension_type,
extensions : vector[offered_psk]
```
            psk_identities : vector[psk_identity],
            psk_binder : stream_data
        }
    }

    object pre_shared_key_server = {
        variant this of extension = struct {
```
etype : extension_type,
extensions : vector[offered_psk]
```
            selected_identity : stream_pos
        }
    }
...
```
    object new_session_ticket = {
        variant this of handshake = struct {
            ticket_lifetime : stream_pos,
            ticket_age_add : stream_pos,
            ticket_nonce : stream_data,
            ticket : stream_data,
            extensions : vector[extension]
        }
    }
