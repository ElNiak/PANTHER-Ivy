...
prover will behave in a more predicatable and stable way.  Ivy's
...
point is that, with undeciable theories, we sometimes fail on even
...
of `quot_rem`, then apply the induciton schema `N` substituted for
the induction variable `X` in the schema. This is sufficint to match
...
numbers less than `x` (the induciton hypothesis). We then split
cases on whether `x < M`. This is easy because the quotient must be
0 and the remainder must be `x`. We 'forget' the induciton
...
for `x-M`. Adding one to the quotient, we get a quotieint/remainder
pair for `x`.
...
GCD. To get Z3 to prove this, we instantiate the definitng property
of GCD. Unfolding the definitions of `is_gcd`, `commddiv` and
...
Notice that in the formal proof, something funy is happening at line
...
second unfolding. This elimantes the variable name clash and gives
...
about GCD's that we will apply to help us prove fcts about prime
factorizations.

Theorem (Bezout's lemma).

If `S,T` are positive, then ther exists ~A,B` such that `A*S-B*T =
gcd(S,T)`.
...
split cases on whether `S > T`, ~S=T` or `S<T`. In the first caxse,
...
Having proved the lemma for all values of `max(S,T)`, we elimiante
...
Proof. By Bezout's lemma, we have `gcd(a,b) = A*a - B*b`. Morover,
...
Actually, we prove the closet thing we can in integer arithetic,
